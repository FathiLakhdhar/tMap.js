(function() {
    ...
    var links = d.append("g").attr("class", "links")
      ...
      , nodes = d.append("g").attr("class", "nodes");
    var graphInfo = d3.select("#graph-info");
    d3.json("metadata.json").then(function(Y) {
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        A = d3.map();
    ...
    function O() {
        ...
        i = Math.floor(height / T.get("episodes").length);
    ...
    function b(X) {
        var X = nodes.selectAll(".node").data(X, u);
    ...
    function V() {
        var X = links.selectAll("path").data(H, u);
    ...
    function D(Y) {
        var Y = episodes.selectAll(".episode").data(Y, u);
    ...
    function m(Y) {
        var X = episodes.selectAll("path").data(Y);
        X.enter().append("path").attr("d", function(Z) {
            return v([[Z.x1, Z.y1], [Z.x1, Z.y1], [Z.x1, Z.y1]])
        }).attr("stroke", "#000").attr("stroke-width", 1.5).transition().duration(w).attr("d", function(Z) {

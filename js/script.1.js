(function() {
    ...
    var d = d3.select("#graph").append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
    var I = d.append("rect").on("click", O);
    I.attr("class", "bg").attr({
        x: width / -2,
        y: height / -2,
        width: width,
        height: height,
    ...
    function O() {
        ...
        var ad = 180 + J
          , Z = 360 - J
          , ac = (Z - ad) / (dataMap.get("themes").length - 1);
        ...
        dataMap.get("episodes").forEach(function(o) {
            o.links.forEach(function(l) {
                ...
                edges.push({
                    ...
                    key: aa,
                    canonicalKey: aa,
                    x1: o.x + (node.type === "theme" ? 0 : U),
                    y1: o.y + K / 2,
                    x2: Math.cos(Y) * X + node.xOffset,
                    y2: Math.sin(Y) * X
                })
            })
        });
    ...
    function updateGraph() {
        ...
        updateNodes(dataMap.get("themes"));
        updateDetail([]);
    ...
    function updateTree() {
        ...
        updateLinks();
        links.selectAll("path").transition().duration(w).attr("d", linkRadial);
        updateEpisodes([]);
        updateNodes(X);
        updateDetail([L.node]);
        var Y = "";
    ...
    function z() {
        ...
        gNodes.selectAll("circle").attr("fill", function(X) {
            if (X === L.node) {
                return "#000"
            } else {
                if (X.type === "theme") {
